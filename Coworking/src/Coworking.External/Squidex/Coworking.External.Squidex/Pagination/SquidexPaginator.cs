...
public sealed class SquidexPaginator : ISquidexPaginator
{
        ...
        CancellationToken ct = default)
    {
        ...
        var remainingPages = await FetchRemainingPagesAsync<T>(
            schema, client, baseQuery, firstPage.Total, pageSize, queryOptions, ct);
    ...
    private static Task<ResponseSchema<T>[]> FetchRemainingPagesAsync<T>(
        string schema,
        ISquidexApiClient client,
        RequestQuery baseQuery,
        long total,
        ...
        CancellationToken ct)
    {
        ...
        var pageTasks = Enumerable.Range(1, pageCount - 1)
            .Select(page => client.QueryAsync<T>(
                schema,
                PageQuery(baseQuery, page, pageSize),
                queryOptions,
                ct));
